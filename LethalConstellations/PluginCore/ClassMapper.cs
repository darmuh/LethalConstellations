...
namespace LethalConstellations.PluginCore
{
    ...
    public class ClassMapper
    {
        ...
        public static bool TryGetConstellation(List<ClassMapper> constellations, string query, out ClassMapper outConst)
        {
            ...
            outConst = constellations.FirstOrDefault(c => c.consName.ToLowerInvariant() == query.ToLowerInvariant() || c.shortcutList.Any(x => x.ToLowerInvariant() == query.ToLowerInvariant()));

            return outConst != null;
        }
        ...
        internal static void ResetUnlockedConstellations(List<ClassMapper> constellations)
        {
            ...
            constellations.Do(c => { c.oneTimePurchase = false; Plugin.Spam($"{c.consName} OTP has been reset"); });
        }
    }
}
